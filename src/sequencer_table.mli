(** A table of sequencers indexed by key, so that at any moment for each key there is at
    ...
    The implementation does not automatically release state; one must call
    [set_state t ~key None]. *)
...
module Make (Key : sig
  ...
  (** Every [Key.t] in the table has an associated [state], which each job running on that
      ...
      to assist debugging, as the tag is included in the sexp serialization of [t]. *)
